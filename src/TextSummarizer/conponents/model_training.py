    ...
    def training_pipeline(self):
        try:
            ...
            trainer_args = TrainingArguments(
                ...
                weight_decay=self.config.weight_decay, logging_steps=self.config.logging_steps,fp16=True,
            ...
            ## Save model
            model.save_pretrained(os.path.join(self.config.root_folder,"trained_model"))
            ## Save tokenizer
            tokenizer.save_pretrained(os.path.join(self.config.root_folder,"trained_tokenizer"))
            logging.info(f"model_and_tokenizer is saved to device")

            # Manually clear GPU cache after each evaluation step
